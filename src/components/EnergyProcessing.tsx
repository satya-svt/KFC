...
export default function EnergyProcessing() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validRows.length === 0) {
      setErrorMessage('Please complete at least one energy entry.')
        ...
        <motion.div className="max-w-md w-full bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-8">
          ...
          >
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              title="Back to Manure Management"
            >
          ...
          <div className="text-center">
            ...
            <p className="text-gray-300 mb-6">Your energy & processing data has been successfully submitted.</p>
                        ...
                        <td className="px-6 py-4">
                          <div className="relative">
                            ...
                            <select
                              value={row.energyType}
                              onChange={(e) => updateEnergyRow(row.index, 'energyType', e.target.value)}
                              className="w-full px-4 py-2 bg-transparent border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all duration-300 appearance-none cursor-pointer"
                            >
                              <option value="" className="bg-gray-800 text-gray-300">Select energy type</option>
                              {energyTypeOptions.map(option => (
                                <option key={option} value={option} className="bg-gray-800 text-white">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-2">
                            <input
                              ...
                              value={row.consumption}
                              onChange={(e) => updateEnergyRow(row.index, 'consumption', e.target.value)}
                              className="flex-1 px-4 py-2 bg-transparent border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all duration-300 text-right"
                              placeholder="Enter amount"
                            />
                          </div>
                        </td>
          ...
          {validRows.length > 0 && (
            ...
            >
              ...
              <p className="text-green-300/80 text-sm">
                {validRows.length} energy {validRows.length === 1 ? 'entry' : 'entries'} configured and ready for submission.
              </p>
            </motion.div>
          )}
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                <span>Complete at least one entry to submit</span>
              </>
            ) : (
              <>
